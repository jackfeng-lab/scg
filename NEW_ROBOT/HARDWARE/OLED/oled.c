...
{ 	
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB  |RCC_AHB1Periph_GPIOC, ENABLE);  //使能GPIOC
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR  , ENABLE);    //PWR使能
	PWR_BackupAccessCmd(ENABLE);         //允许修改RTC 和后备寄存器  
	RCC_LSEConfig(RCC_LSE_OFF);       //关闭外部低速外部时钟信号功能 后，PC13 PC14 PC15 才可以当普通IO用。
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_14 |GPIO_Pin_15;  //PC0 PC13 PC14 PC15
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;   //输出
	GPIO_InitStructure.GPIO_OType =GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed =GPIO_Speed_2MHz;  //2M
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;//根据设定参数初始化GPIO
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_6 |GPIO_Pin_7;  //PC0 PC13 PC14 PC15
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;   //输出
	GPIO_InitStructure.GPIO_OType =GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed =GPIO_Speed_2MHz;  //2M
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;//根据设定参数初始化GPIO
