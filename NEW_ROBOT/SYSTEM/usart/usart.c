...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) //接收到数据
	{
		if(Usart_ON_Flag==0)
		{
			if(++Usart_ON_Count>11)
				;
//				Usart_ON_Flag=1,PS2_ON_Flag=0;
		}
		Usart1_temp=USART1->DR;
		if(Usart_Flag==0)
		{
			if(last_data==0x90&&last_last_data==0x80)
			Usart_Flag=1,count=0;	
		}
		if(Usart_Flag==1)
		{
			Urxbuf[count]=Usart1_temp;
			count++;
			if(count==9)
				Usart_Flag=0;
		}
		last_last_data=last_data;
		last_data=Usart1_temp;
	}
#if SYSTEM_SUPPORT_OS 	//如果SYSTEM_SUPPORT_OS为真，则需要支持OS.
	OSIntExit();
#endif
